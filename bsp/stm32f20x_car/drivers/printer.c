...
{
    printer_param.step_delay	= 1;    //步进延时,影响行间隔
    ...
    {
        printer_param.heat_delay[0] = jt808_param.id_0xF045 + 3;    //加热延时
        printer_param.heat_delay[1] = jt808_param.id_0xF046 + 3;    //加热延时
        printer_param.heat_delay[2] = jt808_param.id_0xF047 + 3;    //加热延时
        printer_param.heat_delay[3] = jt808_param.id_0xF048 + 3;    //加热延时
    }
...
void TIM3_IRQHandler( void )
{
    ...
    case PRINTER_GET_DATA_HEAT_1_3:
        ...
        if(HardWareVerion == HARD_VER_PCB01)
        {
            GLYPH_COL_NUM = GLYPH_COL;
        }
        else
        {
            GLYPH_COL_NUM = GLYPH_COL / 2;
        }
        //for( col_byte = 0; col_byte < GLYPH_COL; col_byte++ )		///384 POINT
        for( col_byte = 0; col_byte < GLYPH_COL_NUM; col_byte++ )		///192 POINT OR 384 POINT
        {
            c = *p++;
            for( b = 0; b < 8; b++ )
            {
...
/*********************************************************************************
*函数名称:void LH_TO_HL_2line(uint8_t *HL_buf,uint8_t *LH_buf,int H_num,int L_num)
*功能描述:将字库编码为列行式转换为行列式，字库单位均为1字节，转换后的字库行数，列数均相同
*输    入:	HL_buf	:保存转换后的字库，转换后为行列式
			LH_buf	:输入的列行式字库
			H_num	:输入字库的行数
			L_num	:输入字库的列数
*输    出:HL_buf	:保存转换后的字库，转换后为行列式
*返 回 值:none
*作    者:白养民
*创建日期:2014-12-29
*---------------------------------------------------------------------------------
...
*修改描述:
*********************************************************************************/
void LH_TO_HL(uint8_t *HL_buf, uint8_t *LH_buf, int H_num, int L_num)
{
    uint8_t	value;
    int i, j;
    int s1, s2, s3;
    ...
    buf_len_source 	= m * L_num;
    buf_len_dest 	= n * H_num;
    memset(HL_buf, 0, buf_len_dest);

    for(i = 0; i < buf_len_source; i++)
    {
        value = LH_buf[i];
        ///计算要存入的新HL式的buf位置
        //先写入的位置"i%m * n * 8"为写入行开始地址，"i/(m*8)"为写入行的地址偏移
        s1 =  i % m * n * 8;
        s3 =  i / (m * 8);
        s2 =  7 - (i / m % 8);		//写入的bit位;
        for(j = 0; j < 8; j++)
...
/*********************************************************************************
*函数名称:void LH_TO_HL_2line(uint8_t *HL_buf,uint8_t *LH_buf,int H_num,int L_num)
*功能描述:将字库编码为列行式转换为行列式，字库单位均为1字节，转换后的字库行数翻倍，列数相同，为了打印需要
*输    入:	HL_buf	:保存转换后的字库，转换后为行列式
			LH_buf	:输入的列行式字库
			H_num	:输入字库的行数
			L_num	:输入字库的列数
...
*修改描述:
*********************************************************************************/
void LH_TO_HL_2line(uint8_t *HL_buf, uint8_t *LH_buf, int H_num, int L_num)
{
    uint8_t	value;
    int i, j;
    int s1, s2, s3;
    ...
    buf_len_source 	= m * L_num;
    buf_len_dest 	= n * H_num;
    memset(HL_buf, 0, buf_len_dest * 2);
    ...
    for(i = 0; i < buf_len_source; i++)
    {
        ...
        for(j = 0; j < 8; j++)
        {
            if(value & (BIT(j)))
            {
                if((s1 + s3 + n) < buf_len_dest * 2)
                {
                    HL_buf[s1 + s3] |= BIT(s2);
                    HL_buf[s1 + s3 + n] |= BIT(s2);
                }
                /*
                else
                {
    ...
    while( charnum )
    {
        ...
        if(HardWareVerion == HARD_VER_PCB01)
        {
            if( msb <= 0x80 ) //ascii字符
            {
...
#ifdef FONT_IS_OUT_FLASH
                sst25_read(addr, font_buf, 48);
                addr = (int)font_buf;
#endif
                ///CPU FLASH
                //addr = ( msb - 0x20 ) * 48 + FONT_ASC1224_ADDR;
                ///将48个字节的字库数据每4个字节一组进行前后调换，然后存入font_glyph[]中
                /*
                ...
                */
                memcpy(font_glyph, (uint8_t *)addr, 48);
            ...
            else
            {
                ...
                */
                ...
                memcpy(font_glyph, (uint8_t *)addr, 72);
                for( row = 0; row < 24; row++ )
                {
        ...
        else //if(HardWareVerion==HARD_VER_PCB02)	//新版本192点阵打印机
        {
            if( msb <= 0x80 ) //ascii字符
            {
                ///OUT FLASH
                ...
                LH_TO_HL_2line(font_glyph, (uint8_t *)addr, 12, 6);
                //memcpy(font_glyph,(uint8_t*)addr,12);

                for( row = 0; row < 24; row++ )
                {
                    val_new = ( font_glyph[row] << 8 );    //读出的字库1byte，只有高6位有效，左对齐

                    col		= start_col >> 3;                                                       //查找所在的列字节 [0..7]对应字节0
                    val_old = ( print_glyph[row][col] << 8 ) | print_glyph[row][col + 1] ;

                    offset						= start_col & 0x07;
                    val_new						>>= offset;
                    //val_mask					= ( 0xFFFFFF00 >> offset );                         //12bit个1的mask
                    //val_ret					= val_old ^ ( ( val_new ^ val_old ) & val_mask );
                    val_ret						= val_old |  val_new ;
                    print_glyph[row][col ]	= ( val_ret >> 8 ) & 0xff;
                    print_glyph[row][col + 1]	= val_ret & 0xff;
                }
                start_col += 6;
            }
            else
            {
                ...
                LH_TO_HL_2line(font_glyph, (uint8_t *)addr, 12, 12);
                //memcpy(font_glyph,(uint8_t*)addr,72);
                for( row = 0; row < 24; row++ )
                {
                    val_new = ( font_glyph[row * 2] << 16 ) | ( font_glyph[row * 2 + 1] << 8 ) ; //读出的字库24byte

                    col		= start_col >> 3;                                                                                       //查找所在的列字节 [0..7]对应字节0
                    val_old = ( print_glyph[row][col] << 16 ) | ( print_glyph[row][col + 1] << 8 ) | print_glyph[row][col + 2];
                    ...
                    //val_ret					= val_old ^ ( ( val_new ^ val_old ) & val_mask );
                    val_ret						= val_old |  val_new;
                    print_glyph[row][col]		= ( val_ret >> 16 ) & 0xff;
                    print_glyph[row][col + 1]	= ( val_ret >> 8 ) & 0xff;
                    print_glyph[row][col + 2]	= val_ret & 0xff;
                }
                start_col += 12;
            }
        }
...
* Others:
***********************************************************/
void printer_get_str_glyph_old( unsigned char *pstr, unsigned char len )
{
    unsigned long	addr;
    unsigned char	charnum = len;
    unsigned char	 *p		= pstr;
