...
#define MIXERS_STOP_NUM		60				///单位为秒，表示多久没有触发传感器就认为搅拌车停止搅拌
...
void mixers_proc( void )
{
    ...
    if((mixers_sensor_tick_BA == 0) || (mixers_sensor_tick_AB == 0))
        return;
    if(mixers_sensor_tick_BA > mixers_sensor_tick_AB * 2)		///B到A的时间比A到B的时间多2倍则表示正传
    {
        ////正转
        mixers_set(1);
    }
    else if(mixers_sensor_tick_AB > mixers_sensor_tick_BA * 2)	///A到B的时间比B到A的时间多2倍则表示反传
    {
        ////反转
        mixers_set(2);
    }
